...
#'''
 ...
 #           self.ball_not_seen_frames += 1
        ...
        x_diff = rob_x - (-1500 + 325)

        keep_center_gain = 0.001
        #keep_center_gain = 0.0


        commands.setWalkVelocity(-x_diff*keep_center_gain, y_diff*keep_center_gain, -theta_diff*0.0275)
        ...
        if x_pos - rob_x > 750 or not ball.seen:
            if self.getFrames() % rescan == 0 or self.start_scan != -1:
                ...
                commands.setHeadPan(-math.pi / 10, 0.10)
                if self.getFrames() - self.start_scan > 20: #0.5 sec at 100Hz (min 10)
                    commands.setHeadPan(math.pi / 20, 0.4)
